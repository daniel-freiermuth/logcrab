...
impl LogFileLoader {
    ...
    pub fn load_async(
        ...
        let (tx, rx) = channel();
        let data_source = Arc::new(SourceData::from_file(path.clone()));
    ...
    fn read_generic_file(
        ...
        for line_buffer in content.lines() {
            ...
            if chunk_lines.len() >= CHUNK_SIZE {
                ...
                let progress = (bytes_read as f32 / file_size as f32).min(1.0);
                source.set_loading_progress(progress);
        ...
        if !chunk_lines.is_empty() {
            ...
            source.set_loading_progress(1.0);
        }
    ...
    fn score_and_send_lines(
        ...
        for (idx, log_line) in data_source.iter_all().enumerate() {
            if idx % 1000 == 0 {
